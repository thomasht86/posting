...
class DocSearchView(Horizontal):
    ...
    def _on_mount(self):
        ...
        self.watch(self.query_one(selector="#chat-response"), "text", self.query_one(selector="#chat-response").watch_text)
    ...
    def get_filter_string(self) -> str:
        ...
        return " ".join([self.FILTERS[filter_name] for filter_name, is_active in self.BUTTON_STATES.items() if is_active])

    @work(exclusive=True)
    async def send_chat_via_worker(self) -> None:
        await self.send_chat_request(query=self.search_query, filter_string=self.get_filter_string())
    
    @work(exclusive=True)
    async def send_search_via_worker(self) -> None:
        await self.send_search_request(query=self.search_query, filter_string=self.get_filter_string())
    ...
    async def send_chat_request(self, query: str, filter_string: str) -> None:
        ...
        async with httpx.AsyncClient() as client:
            try:
                response = await client.get(self.chat_url, headers=self.chat_request_headers, params=params)
    ...
    async def send_search_request(self, query: str, filter_string: str) -> None:
        ...
        async with httpx.AsyncClient() as client:
            try:
                response = await client.get(self.search_url, headers=self.search_request_headers, params=params)
                response.raise_for_status()
                resp = response.json()
    ...
    def abstract_markdown(self) -> Markdown:
        return self.query_one(selector="#chat-response", expect_type=self.AbstractMarkdown)